...
/**
 * Interfaz para las propiedades del componente InferentialAnalysis.
 * * @interface InferentialProps
 * @property {string[]} headers - Lista de nombres de las columnas (encabezados) del archivo procesado.
 * @property {(var1: string, var2: string) => void} onRunAnalysis - Función callback que dispara el procesamiento estadístico.
 * @property {InferentialResult[]} results - Arreglo con los resultados obtenidos tras el análisis.
 * @property {boolean} isProcessing - Estado que indica si los cálculos están en ejecución.
 */
interface InferentialProps {
  headers: string[];
...
/**
 * Componente que gestiona la interfaz de usuario para la Estadística Inferencial.
 * Permite seleccionar variables y visualizar resultados de estimación, regresión y correlación.
 * * @component
 */
export const InferentialAnalysis: React.FC<InferentialProps> = ({ headers, onRunAnalysis, results, isProcessing }) => {
  // Estados locales para almacenar las variables seleccionadas por el usuario
  const [var1, setVar1] = useState<string>('');
  const [var2, setVar2] = useState<string>('');

  /**
   * Maneja la ejecución del análisis.
   * Valida que al menos la variable principal esté seleccionada antes de invocar la lógica superior.
   */
  const handleRun = () => {
    if (var1) {
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      {/* Sección de Configuración: Selección de Variables */}
      <div className="bg-white shadow rounded-lg p-6 border-l-4 border-brand-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
          ...
          {/* Selector para la Variable Secundaria (Opcional) */}
          <div>
            ...
            >
              ...
              {/* Filtramos var1 para evitar que el usuario seleccione la misma variable dos veces */}
              {headers.filter(h => h !== var1).map(h => <option key={h} value={h}>{h}</option>)}
            </select>
          </div>

          {/* Botón de Acción para disparar los motores de cálculo */}
          <button
            onClick={handleRun}
        ...
        <div className="grid grid-cols-1 gap-6">
          {results.map((res, idx) => (
            <div key={idx} className="bg-white overflow-hidden shadow rounded-lg border border-slate-200">
              {/* Cabecera de la tarjeta de resultado */}
              ...
              <div className="p-4">
                  ...
                  {/* Grid de métricas numéricas generadas */}
                  ...
                  {/* Interpretación automática (Conclusión) */}
                  {res.conclusion && (
